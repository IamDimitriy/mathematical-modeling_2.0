...
class Bacteria(pygame.sprite.Sprite):
    ...
    def __init__(self, resist, immunity, crispr_cas_st, crispr, img, x, y):
        ...
        self.rect.x = x
        self.rect.y = y
        mutation = 0  # random.uniform(-0.005, 0.005)
        self.resist = resist+mutation
        self.immunity = immunity+mutation
        self.st_crispr_cas = crispr_cas_st
        self.crispr = crispr
        self.lifetime = 0  # время жизни в шагах итерационного процесса.
        self.last_move = 0
            ...
            if penetration >= 0:
                ...
                if not fac.color in self.crispr:
                    if infection >= 0:
                        ...
                        img = dir/"images"/"sprites"/"bacs"/"RED_bac.png"
            ...
            if penetration >= 0:
                ...
                if not fac.color in self.crispr:
                    if infection >= 0:
                        ...
                        quantity = int(abs(1-infection * 10*2))
                        self.kill()
                        img = dir/"images"/"sprites"/"bacs"/"YELLOW_bac.png"
                        bac = Bacteria(self.resist, self.immunity, False,
                                       crispr_def, img, x, y)
                        inf_bacs.add(bac)
                        all_sprites.add(bac)
            ...
            if penetration >= 0:
                ...
                if not fac.color in self.crispr:
                    if infection >= 0:
                        ...
                        quantity = int(abs(1-infection * 10*2))
                        self.kill()
                        img = dir/"images"/"sprites"/"bacs"/"VIOLET_bac.png"
                        bac = Bacteria(self.resist, self.immunity, False,
                                       crispr_def, img, x, y)
                        inf_bacs.add(bac)
                        all_sprites.add(bac)
    ...
    def update(self):
        ...
        if self.eng < 0:
            if self.img == img_normal:
                ...
                all_sprites.add(eat)
                eats.add(eat)
            elif self.img == dir/"images"/"sprites"/"bacs"/"RED_bac.png":
                self.kill()
                x = self.rect.x
                y = self.rect.y
                for i in range(0, self.quantity):
                    ...
                    red_facs.add(fac)
                    i += 1
            elif self.img == dir/"images"/"sprites"/"bacs"/"YELLOW_bac.png":
                self.kill()
                x = self.rect.x
                y = self.rect.y
                for i in range(0, self.quantity):
                    ...
                    red_facs.add(fac)
                    i += 1
            elif self.img == dir/"images"/"sprites"/"bacs"/"VIOLET_bac.png":
                self.kill()
                x = self.rect.x
                y = self.rect.y
                for i in range(0, self.quantity):
...
class Bacteriophage(pygame.sprite.Sprite):
    def __init__(self, color, x, y):
        self.ingress = random.uniform(0.4, 1.2)
        self.suppression = random.uniform(0.4, 1.2)
        pygame.sprite.Sprite.__init__(self)
        self.color = color
        if self.color == RED:
            self.img = dir/"images"/"sprites"/"facs"/"RED_fac.png"
        if self.color == VIOLET:
            self.img = dir/"images"/"sprites"/"facs"/"VIOLET_fac.png"
        if self.color == YELLOW:
            self.img = dir/"images"/"sprites"/"facs"/"YELLOW_fac.png"
        self.image = pygame.image.load(self.img).convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
...
class Eat(pygame.sprite.Sprite):
    def __init__(self, color_eat, x, y):
        ...
        pygame.sprite.Sprite.__init__(self)
        if color_eat == GREEN:
            self.img = dir/"images"/"sprites"/"eat.png"
            self.image = pygame.image.load(self.img).convert_alpha()
        else:
            color = color_eat
